...
# Constantes para checkIfPossibleChar, checa apenas um possível caractere (não compara com outros)
MIN_PIXEL_WIDTH = 2
MIN_PIXEL_HEIGHT = 8
...
def loadKNNDataAndTrainKNN():
    ...
    try:
        ...
        print "Erro: Não foi possível abrir flattened_images.txt.\n"               # show error message
        np.os.system("pause")
        return False                                                                        # and return False
    # end try
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        if (len(listOfListsOfMatchingCharsInPlate) == 0):			# if no groups of matching chars were found in the plate
            ...
            if Main.showSteps == True: # show steps ###############################################
                print "Caracteres encontrados na placa " + str(intPlateCounter) + " = (nenhum), Clique em qualquer imagem e pressione uma tecla para continuar."
        ...
        if Main.showSteps == True: # show steps ###################################################
            print "Caracteres encontrados na placa " + str(intPlateCounter) + " = " + possiblePlate.strChars + ". Clique em qualquer imagem e pressione uma tecla para continuar."
            intPlateCounter = intPlateCounter + 1
            cv2.waitKey(0)
        # end if # show steps #####################################################################
    ...
    if Main.showSteps == True:
        print "\nDetecção de caracteres concluída. Clique em qualquer imagem e pressione uma tecla para continuar.\n"
        cv2.waitKey(0)
    # end if
